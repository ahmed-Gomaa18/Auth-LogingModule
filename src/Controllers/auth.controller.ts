import {
  ...
  keycloakSignupService,
  keycloakResetPasswordService,
...
export async function Register(req: Request, res: Response) {
  try {
    ...
    // Send Mail
    sendConfirmationMail(result.user._id, result.user.email, req);
    // Logging
...
export async function Login(req: Request, res: Response) {
  ...
  try {
    ...
    if (result.isSuccess) {
      Logger.info(
        `UserId: (${result.user._id}) email: (${result.user.email}) LogIn Successfully.`,
        { req }
      );
    ...
    Logger.error(
      `Error Occurred While This User Try to LogIn email:(${email}) Error: ${error.message}.`,
      { req }
    );
...
export async function loginByKeycloak(
  ...
  try {
    ...
    if (result.isSuccess) {
      ...
      Logger.error(
        `This email(${email}) Tried To login with Error (${result.message})`,
        { req }
      );
      return res.status(result.status).json({ message: result.message });
    }
...
export async function registerByKeycloak(
  ...
  try {
    const result = await keycloakSignupService({
      email,
      firstName,
      lastName,
      password,
    ...
    Logger.error(
      `This email(${email}) Tried To Sign up By Keycloak.\nError(${error.message})`,
      { req }
    );
    res.status(500).json({
      message:
        "Catch Error" +
        "Oops.. Some Error Occurred While sign up" +
        error.message,
    });
  }
}

export async function resetPasswordByKeycloak(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  try {
    ...
    Logger.error(
      `This email(${email}) Tried To Reset Password.\nError(${error.message})`,
      { req }
    );
    res.status(500).json({
      message:
        "Catch Error" +
        "Oops.. Some Error Occurred While reset password" +
        error.message,
    });
  }
}
...
// Auth By Google
export function httpLoginByGoogle(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export function httpCallbackGoogleURL(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
// Auth By Facebook
export function httpLoginByFacebook(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
export function httpCallbackFacebookURL(
  req: Request,
  res: Response,
  next: NextFunction
) {
...
// Auth by Keycloak
export function httpLoginByKeycloak(
  req: Request,
  res: Response,
  next: NextFunction
) {
  passport.authenticate("keycloak", {session: false})(req, res, next);
}

export function httpCallbackKeycloakURL(
  req: Request,
  res: Response,
  next: NextFunction
) {
  passport.authenticate("keycloak", { session: false }, (err, user) => {
    if (err) {
      Logger.error(
        `This UserId (${user?.userinfo?.sub}) is failed to Authenticated by Keycloak`,
        { req }
      );
      return next(err);
    }
    Logger.info(`This UserId (${user._id}) is Authenticated by Keycloak`, {
      req,
    });
    console.log({user})
